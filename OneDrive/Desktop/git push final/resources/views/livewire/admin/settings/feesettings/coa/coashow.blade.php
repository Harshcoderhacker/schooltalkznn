@if ($isshowmodalopen && $coashowdata)
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header bg-primary">
                        <h2 class="font-bold text-lg text-white mr-auto">COA</h2>
                        <button wire:click="coaclosemodal"
                    ...
                    <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                        ...
                        ['label'=> 'ID',
                        'value'=> $coashowdata->uniqid])

                        @include('helper.show.show',
                        ['label'=> 'NAME',
                        'value'=> $coashowdata->name])

                        @include('helper.show.show',
                        ['label'=> 'CREATED BY',
                        'value'=> $coashowdata->created_by])

                        @include('helper.show.show',
                        ['label'=> 'CREATED AT',
                        'value'=> $coashowdata->created_at->format('d-M-Y h:i:s')])

                        @if ($coashowdata->updated_by)
                            @include('helper.show.show',
                            ['label'=> 'UPDATED BY',
                            'value'=> $coashowdata->updated_by])

                            @include('helper.show.show',
                            ['label'=> 'UPDATED AT',
                            'value'=> $coashowdata->updated_at->format('d-M-Y h:i:s')])
                        @endif

                    </div>
                    <div class="modal-footer text-right">
                        <button type="button" wire:click="coaclosemodal"
