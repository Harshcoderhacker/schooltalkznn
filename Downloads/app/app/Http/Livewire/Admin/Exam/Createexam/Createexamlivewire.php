...
{
    public $show = 1;
    public $is_main_exam = false;
    public $validatedconfigureclass, $academicyear_id;
    public $classmaster, $sectionlist = [], $name, $classmaster_name, $section_name;
    public $classmaster_id, $section, $exam_id;
    ...
    public function mount($exam_id = null, $show)
    {
        if ($exam_id) {
            ...
            $this->sectionlist = $exam->classmaster->section;
    ...
    public function validateconfigureclass()
    {
        ...
        $this->validatedconfigureclass['is_main_exam'] = $this->is_main_exam;
        $this->subject = Assignsubject::where('active', true)
            ->where('classmaster_id', $this->classmaster_id)
            ->whereIn('section_id', $this->section)
            ->get();
    ...
    public function submitexam()
    {
        try {
            ...
            Helper::trackmessage(auth()->user(), 'Admin Exam Create/Edit', 'admin_web_exam_create/edit', session()->getId(), 'WEB');
            DB::commit();
            $this->dispatchBrowserEvent('successtoast', ['message' => 'Exam Created Successfully!']);
            redirect()->route('admincreateexamindex');
    ...
    public function createoreditsubjectmark($exam)
    {
        ...
        $examsubject = Examsubject::where('exam_id', $exam->id)->select('examdate')->get();
        $exam->update(['month' => $examsubject->min('examdate')->format('m')]);
    ...
    {
        return view('livewire.admin.exam.createexam.createexamlivewire');
    }
