...
@section('content')
    ...
    <div class="flex">
        ...
        <nav class="side-nav">
            <a href="" class="intro-x flex items-center pl-5 pt-4">
                <img alt="Edfish School ERP Software" class="w-24"
                    src="{{ asset('/image/logo/white_logo.png') }}">
                {{-- <span class="hidden xl:block text-white text-lg ml-3">
                E<span class="font-medium">dfish</span>
            </span> --}}
            </a>
            <div class="side-nav__devider my-6"></div>
            <ul>
                @foreach ($staff_side_menu as $menuKey => $menu)
                    @if ($menu == 'devider')
                        <li class="side-nav__devider my-6"></li>
                    @else
                        <li>
                            <a href="{{ isset($menu['route_name']) ? route($menu['route_name'], $menu['params']) : 'javascript:;' }}"
                                id={{ isset($menu['route_name']) ? $menu['route_name'] : 'javascript:;' }}
        ...
        <!-- BEGIN: Content -->
        <div class="content">
            @include('layout.staff.components.top-bar')
            @yield('subcontent')
        </div>
        <!-- END: Content -->
    </div>
