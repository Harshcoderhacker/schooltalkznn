<div>
    <div class="intro-y chat grid grid-cols-12 gap-5 mt-5 mb-5">
        @include('admin.settings.schoolsettings.helper.schoolsettingsmenu', ['active' => 'language'])
        <div class="col-span-12 xl:col-span-4 mt-4">
            <div class="intro-y block sm:flex items-center h-10">
                <h2 class="text-lg font-bold text-gray-900 dark:text-gray-100 mr-5">Add Language</h2>
            </div>
                ...
                <form wire:submit.prevent="createorupdate">
                    <div class="relative text-gray-700 dark:text-gray-300">
                        <input wire:model="name" type="text"
                            ...
                            placeholder="Name*" id="language_field_focus" autocomplete="off">
                        @error('name') <span class="pristine-error text-red-600 mt-2">{{ $message }}</span>
                        @enderror
                    </div>
                    @if ($languageid)
                        <div class="flex mt-3 gap-2 justify-center">
                            <button type="submit" {{ $languagesubmitbtn ? 'disabled' : '' }}
                                class="btn btn-primary">Update Language</button>
                            <a wire:click="formcancel" class="btn btn-danger">Cancel</a>
                        </div>
                    @else
                        <button type="submit" {{ $languagesubmitbtn ? 'disabled' : '' }}
                            class="btn btn-primary w-full mt-3">Add Language</button>
                    @endif
                </form>
            </div>
        </div>
            ...
            <div class="p-2">
                <div class="grid grid-cols-12 gap-1 ">
                    ...
                    @if ($language->isNotEmpty())
                            ...
                            <table class="table table-report -mt-2">
                                ...
                                <tbody>
                                    @foreach ($language as $index => $value)
                                        <tr class="intro-x">
                                            <td class="">{{ $language->firstItem() + $index }}</td>
                                            <td>
                        ...
                        @include('helper.datatable.pagination', ['pagination' => $language])
                    @elseif($searchTerm && $language->isEmpty())
                    ...
                    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center p-10 box mt-4 bg-blue-100 leading-6">
                        <div class="mx-auto flex flex-row items-center">
                            <div>
                                ...
                                <p class="text-2xl mt-2 font-bold"> <span class="text-green-500">Language Name</span></p>
                                <p class="text-2xl mt-2 font-bold text-gray-900 dark:text-gray-100 dark:text-white mr-5">to create language</p>
                            </div>
                            <div>
    ...
    @include('livewire.admin.settings.schoolsettings.language.languageshow')
</div>
