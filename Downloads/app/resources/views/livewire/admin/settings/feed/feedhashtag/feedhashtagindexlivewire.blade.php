<div>
    <div class="intro-y chat grid grid-cols-12 gap-5 mt-5 mb-12">
        @include('admin.settings.feedsettings.helper.feedsettingsmenu', ['active' => 'feedtag'])
                    ...
                    @if ($feedtag->isNotEmpty())
                        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                            ...
                            <table class="table table-report sm:mt-2">
                                ...
                                <tbody>
                                    @foreach ($feedtag as $index => $value)
                                        <tr class="intro-x">
                                            <td class="font-medium whitespace-nowrap">
                                                {{ $feedtag->firstItem() + $index }}</td>
                                            <td class="font-medium whitespace-nowrap">{{ $value->name }}</td>
                                            <td class="font-medium whitespace-nowrap">
                                                <span class="font-bold">{{ $value->feedpost()->count() }}</span> posts
                                            </td>
                        ...
                        @include('helper.datatable.pagination', ['pagination' => $feedtag])
                    @else
