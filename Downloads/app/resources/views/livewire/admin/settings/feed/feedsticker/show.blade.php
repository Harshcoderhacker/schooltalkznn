@if ($isshowmodalopen && $stickershowdata)
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header bg-primary">
                        <h2 class="font-bold text-lg text-white mr-auto">Feed Sticker</h2>
                        <button wire:click="stickerclosemodal"
                    ...
                    <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                        ...
                        @include('helper.show.show', [
                            'label' => 'Sticker Name',
                            'value' => $stickershowdata->name,
                        ])

                        @include('helper.show.show', [
                            'label' => 'STICKER CATEGORY',
                            'value' => $stickershowdata->sticker_category
                                ? config('archive.sticker_category')[$stickershowdata->sticker_category]
                                : '',
                        ])
                        <div class="col-span-12 sm:col-span-6">
                            <label class="form-label">STICKER</label><br>
                            @if ($stickershowdata->sticker_path)
                                <img class="img-fluid" style="width: 70px; height:60px;"
                                    src="{{ url('/storage/' . $stickershowdata->sticker_path) }}">
                            @endif
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-footer text-right">
        <button type="button" wire:click="stickerclosemodal"
