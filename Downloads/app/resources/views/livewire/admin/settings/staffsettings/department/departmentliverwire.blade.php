<div>
    <div class="intro-y chat grid grid-cols-12 gap-5 mt-5 mb-5">
        @include('admin.settings.staffsettings.helper.staffsettingsmenu', ['active' => 'staffdepartment'])
    </div>
    <div class="intro-y chat grid grid-cols-12 gap-5 ">
        <div class="col-start-2 col-span-12 xl:col-span-4 mt-4">
            <div class="intro-y block sm:flex items-center h-10">
                <h2 class="text-lg font-bold text-gray-900 dark:text-gray-100 mr-5">Add Department</h2>
            </div>
                ...
                <form wire:submit.prevent="createorupdate">
                    <div class="relative text-gray-700 dark:text-gray-300">
                        <input wire:model="name" type="text"
                            ...
                            placeholder="Name*" id="staffdepartment_field_focus" autocomplete="off">
                    ...
                    @if ($staffdepartmentid)
                        <div class="flex mt-3 gap-2 justify-center">
                            <button type="submit" {{ $departmentsubmitbtn ? 'disabled' : '' }}
                                class="btn btn-primary">Update Department</button>
                            <a wire:click="formcancel" class="btn btn-danger">Cancel</a>
                        </div>
                    @else
                        <button type="submit" {{ $departmentsubmitbtn ? 'disabled' : '' }}
                            class="btn btn-primary w-full mt-3">Add Department</button>
                    @endif
                </form>
            </div>
        </div>
                    ...
                    @if ($staffdepartment->isNotEmpty())
                        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                            <table class="table table-report -mt-2">
                                ...
                                <tbody>
                                    @foreach ($staffdepartment as $index => $value)
                                        <tr class="intro-x">
                                            <td class="">{{ $staffdepartment->firstItem() + $index }}
                                            </td>
                                            <td>
                                                <a href=""
                                            ...
                                            <td class="table-report__action w-56">
                                                <div class="flex justify-center gap-2 items-center">
                                                    ...
                                                    ['modalname' => 'staffdepartment-show',
                                                    'method' => 'show',
                                                    'id' => $value->id])
                        ...
                        @include('helper.datatable.pagination', ['pagination' => $staffdepartment])
                    @elseif($searchTerm && $staffdepartment->isEmpty())
                    ...
                    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center p-10 box mt-4 bg-blue-100 leading-6">
                        <div class="mx-auto flex flex-row items-center">
                            <div>
                                ...
                                <p class="text-2xl mt-2 font-bold"> <span class="text-green-500">Department Name</span></p>
                                <p class="text-2xl mt-2 font-bold text-gray-900 dark:text-gray-100 dark:text-white mr-5">to create department</p>
                            </div>
                            <div>
    ...
    @include('livewire.admin.settings.staffsettings.department.departmentshow')
</div>
