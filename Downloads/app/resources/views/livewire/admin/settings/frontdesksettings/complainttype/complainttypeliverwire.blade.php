<div>
    <div class="intro-y chat grid grid-cols-12 gap-5 mt-5 mb-5">
        @include('admin.settings.frontdesksettings.helper.frontdesksettingsmenu', ['active' => 'complainttype'])
    </div>
    <div class="intro-y chat grid grid-cols-12 gap-5 ">
        <div class="col-start-2 col-span-12 xl:col-span-4 mt-4">
            <div class="intro-y block sm:flex items-center h-10">
                <h2 class="text-lg font-bold text-gray-900 dark:text-gray-100 mr-5">Add Complaint Type</h2>
            </div>
                ...
                <form wire:submit.prevent="createorupdate">
                    <div class="relative text-gray-700 dark:text-gray-300">
                        <input wire:model="name" type="text"
                            ...
                            placeholder="Name*" id="complainttype_field_focus" autocomplete="off">
                        @error('name') <span class="pristine-error text-red-600 mt-2">{{ $message }}</span>
                        @enderror
                    </div>
                    @if ($complainttypeid)
                        <div class="flex mt-3 gap-2 justify-center">
                            <button type="submit" {{ $complainttypesubmitbtn ? 'disabled' : '' }}
                                class="btn btn-primary">Update Complaint Type</button>
                            <a wire:click="formcancel" class="btn btn-danger">Cancel</a>
                        </div>
                    @else
                        <button type="submit" {{ $complainttypesubmitbtn ? 'disabled' : '' }}
                            class="btn btn-primary w-full mt-3">Add Complaint Type</button>
                    @endif
                </form>
            </div>
        </div>
        <div class=" col-span-12 xl:col-span-8 ">
            <div class="p-2">
                <div class="grid grid-cols-12 gap-1">
                    ...
                    ['title' => 'Complaint Type List',
                    'search' => 'searchTerm'])
                    <!-- BEGIN: Data List -->
                    @if ($complainttype->isNotEmpty())
                        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                            <table class="table table-report -mt-2">
                                ...
                                <tbody>
                                    @foreach ($complainttype as $index => $value)
                                        <tr class="intro-x">
                                            <td>{{ $complainttype->firstItem() + $index }}</td>
                                            <td>
                        ...
                        @include('helper.datatable.pagination', ['pagination' => $complainttype])
                    @elseif($searchTerm && $complainttype->isEmpty())
                    ...
                    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center p-10 box mt-4 bg-blue-100 leading-6">
                        <div class="mx-auto flex flex-row items-center">
                            <div>
                                ...
                                <p class="text-2xl mt-2 font-bold"> <span class="text-green-500">Complaint Type Name</span></p>
                                <p class="text-2xl mt-2 font-bold text-gray-900 dark:text-gray-100 dark:text-white mr-5">to create complaint type</p>
                            </div>
                            <div>
    ...
    @include('livewire.admin.settings.frontdesksettings.complainttype.complainttypeshow')
</div>
