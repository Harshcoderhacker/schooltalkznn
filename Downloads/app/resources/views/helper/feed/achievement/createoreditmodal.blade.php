...
<div class="right-0 left-0 justify-center items-center h-modal md:h-full inset-0 fixed pin overflow-auto bg-smoke-dark flex"
    ...
    <div class="bg-white rounded-lg dark:bg-gray-700 w-full max-w-lg shadow-2xl">
        ...
        <div class="p-6 space-y-6">
            <div>
                <label for="archivement_post" class="form-label font-medium">Post</label>
                <textarea autocomplete="off" wire:model.lazy="archivement_post" name="archivement_post"
                    id="archivement_post" type="text" class="form-control" rows="5" style="resize:none;"
                    placeholder="Share your thoughts over here. Get to know your peers opinions">
                        </textarea>
                @error('archivement_post') <span class="font-semibold text-red-600 mt-2">{{ $message
                    }}</span>
                @enderror
            </div>
        </div>
        <div
            ...
            @else
            <button wire:click="createpost(2, true)" class="btn btn-primary">Post</button>
            @endif
        </div>
    </div>
</div>